define([ 'core/mediaManager' ], function(MediaManager)
{
	...
	this._bindUIEventsUploadFile($("#uploadForms"), $("#chooseFile"));
	...
	// var resourceFiles = formRootElement.find("#image_annotator_image_resource_file_file");
	var title = formRootElement.find("#image_annotator_image_media_titles");
...
//		newTitle.children('input').eq(0).val(MediaChooser._cleanFileNameNoExt(resourceFile.get(0).files[0].name));
	...
	/*
	 * if (jQuery.isFunction(this.callbacks.dialogClose)) { this.callbacks.dialogClose(this.media); }
	 */
